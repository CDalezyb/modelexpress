...
def test_the_stamp_does_not_disturb_the_shard_table():
    ...
    stamped = unwrap_rendezvous_blob_with_step(_blob(step=5, tensors=tensors))
    plain = unwrap_rendezvous_blob_with_step(_blob(tensors=tensors))

    assert stamped[:4] == plain[:4]
    assert stamped[3][0].shards[0].addr == 4096
